...
//            String weekSimple = weeksSimple[i];
    ...
    private void initTimeView(View view) {
        ...
        for (int i = 0; i < 24; i++) {
            hours.add(AppUtil.formatString("%02d", i));
        }
        for (int i = 0; i < 60; i++) {
            mins.add(AppUtil.formatString("%02d", i));
        }
    ...
    private void saveSuccess() {
        ...
        showTips(R.string.save_alarm_success);
    ...
    private void delAlarmActual() {
        mRCSPController.deleteAlarm(mTargetDevice, alarmBean, new OnRcspActionCallback<Boolean>() {
            @Override
            public void onSuccess(BluetoothDevice device, Boolean message) {
                showTips(R.string.alarm_delete_success);
            ...
            @Override
            public void onError(BluetoothDevice device, BaseError error) {
                showTips(R.string.alarm_delete_failure);
            }
        });
    }
