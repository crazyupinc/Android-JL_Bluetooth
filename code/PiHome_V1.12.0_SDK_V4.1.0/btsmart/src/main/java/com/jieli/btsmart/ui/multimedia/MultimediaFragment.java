...
import com.jieli.btsmart.ui.multimedia.control.fm.FMControlFragment;
import com.jieli.btsmart.ui.multimedia.control.fm.FMTXControlFragment;
...
import com.jieli.btsmart.ui.multimedia.control.id3.ID3EmptyControlFragment;
import com.jieli.btsmart.ui.multimedia.control.linein.LineInControlFragment;
import com.jieli.btsmart.ui.multimedia.control.music.BlankMusicControlFragment;
import com.jieli.btsmart.ui.multimedia.control.music.MusicControlFragment;
import com.jieli.btsmart.ui.multimedia.control.net_radio.NetRadioControlFragment;
 ...
 */
@RuntimePermissions
public class MultimediaFragment extends BaseFragment implements IMultiMediaContract.IMultiMediaView {

    public static final int INDEX_SEARCH_DEVICE = 0;
    public static final int INDEX_NET_RADIO = 1;
    ...
    private void initView(@NonNull View root) {
        ...
        if (!mPresenter.isDevConnected()) {
            mAdapter.setList(getFunctionItemDataList());
        }
        ...
        mAdapter.setOnItemClickListener((adapter, view1, position) -> {
            ...
            if (!mPresenter.isDevConnected()) {
                if (SConstant.ALLOW_SWITCH_FUN_DISCONNECT) {
                    ...
                    return;
                } else if (itemData.getItemType() != FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_SEARCH_DEVICE) {
...
//                    switchTopControlFragment(true, AttrAndFunCode.SYS_INFO_FUNCTION_FM);
                ...
                case FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_LIGHT_SETTINGS:
                    ContentActivity.startActivity(requireContext(), LightContainerFragment.class.getCanonicalName(), getString(R.string.multi_media_light_settings));
                    break;
                case FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_ALARM:
                    ContentActivity.startActivity(requireContext(), AlarmListFragment.class.getCanonicalName(), getString(R.string.multi_media_alarm));
                    break;
                case FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_SEARCH_DEVICE:
                    tryToRequestLocationPermission(INDEX_SEARCH_DEVICE);
                    break;
                case FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_NET_RADIO:
                    tryToRequestLocationPermission(INDEX_NET_RADIO);
                    break;
                case FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_SOUND_CARD:
                    ContentActivity.startActivity(requireContext(), SoundCardFragment.class.getCanonicalName(), R.string.multi_media_sound_card);
                    break;
//                case FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_SPDIF:
//                    mPresenter.switchToSPDIFMode();
//                    break;
//                     @note 暂时关闭支持PC从机功能
//                case FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_PC_SLAVE:
//                    mPresenter.switchToPCSlaveMode();
//                    break;
            }
        });
    }
    ...
    public void onStorageDenied() {
        disPermissionTipsDialog();
        UIHelper.showAppSettingDialog(MultimediaFragment.this, getString(R.string.permissions_tips_02) + getString(R.string.permission_storage));
    }
    ...
    public void showRelationForLocationPermission(PermissionRequest request) {
        disPermissionTipsDialog();
        showLocationPermissionTipsDialog(request, index);
    }
    ...
    public void onLocationDenied() {
        disPermissionTipsDialog();
        UIHelper.showAppSettingDialog(MultimediaFragment.this, getString(R.string.permissions_tips_02) + getString(R.string.permission_location));
    }
    ...
    private void goToLocalMusicFragment() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            if (!PermissionUtil.isHasPermission(requireContext(), Manifest.permission.READ_MEDIA_AUDIO)) {
                showPermissionTipsDialog(getString(R.string.music_function_storage_permission));
    ...
    private void toLocalMusicFragment() {
        if (mPresenter instanceof MultiMediaPresenterImpl) {
            ((MultiMediaPresenterImpl) mPresenter).initPlayControl();
        }
    ...
    private void tryToRequestLocationPermission(int index) {
        if (!PermissionUtil.isHasLocationPermission(requireContext())) {
            if (index == INDEX_NET_RADIO) {
                showPermissionTipsDialog(getString(R.string.net_radio_location_permission));
            } else {
                showPermissionTipsDialog(getString(R.string.search_device_location_permission));
            }
            this.index = index;
            MultimediaFragmentPermissionsDispatcher.goToFragmentByTypeWithPermissionCheck(this, index);
            return;
        }
        toNeedLocationPermissionFragment(index);
    }

    private void toNeedLocationPermissionFragment(int index) {
        if (index == INDEX_NET_RADIO) {
    ...
    private void showLocationPermissionTipsDialog(PermissionRequest request, int type) {
        String content = (type == INDEX_NET_RADIO ? getString(R.string.net_radio_location_permission)
                : getString(R.string.search_device_location_permission));
    ...
    private void showStoragePermissionDialog(PermissionRequest request) {
        Jl_Dialog dialog = Jl_Dialog.builder()
                .title(getString(R.string.dialog_tips))
                .content(getString(R.string.music_function_storage_permission))
                .right(getString(R.string.setting))
                .rightColor(ContextCompat.getColor(requireContext(), R.color.blue_448eff))
                .rightClickListener((v, dialogFragment) -> {
                    dialogFragment.dismiss();
    ...
    private void handleItemClickOnTest(int type) {
        ...
        switch (type) {
            ...
            case FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_SOUND_CARD:
                ...
                break;
//            case FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_SPDIF:
//                switchTopControlFragment(true, AttrAndFunCode.SYS_INFO_FUNCTION_SPDIF);
//                break;
//                   @note 暂时关闭支持PC从机功能
//            case FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_PC_SLAVE:
//                switchTopControlFragment(true, AttrAndFunCode.SYS_INFO_FUNCTION_PC_SLAVE);
//                break;
        }
    }
    ...
    public void updateFunctionItemDataList(ArrayList<FunctionItemData> arrayList) {
        if (isInvalid()) return;
        JL_Log.d(TAG, "updateFunctionItemDataList", "size : " + arrayList.size());
    ...
    public void switchTopControlFragment(boolean isConnect, byte fun) {
        ...
        if (!isConnect) {
            ...
            final DeviceInfo deviceInfo = mPresenter.getDeviceInfo();
            if (fun == AttrAndFunCode.SYS_INFO_FUNCTION_BT && deviceInfo != null
                    && (!deviceInfo.isBtEnable() || deviceInfo.isSupportDoubleConnection()
            ...
            switch (fun) {
                ...
                case AttrAndFunCode.SYS_INFO_FUNCTION_LOW_POWER://低功耗模式
                    List<Music> dataList = mPresenter.getLocalMusicList(requireContext());
                ...
                case MultiMediaPresenterImpl.FRAGMENT_NET_RADIO:
                    ...
                    break;
                case AttrAndFunCode.SYS_INFO_FUNCTION_SPDIF:
//                    if (mCurrentControlFragment instanceof SPDIFControlFragment) return;
                    fragment = SPDIFControlFragment.newInstanceForCache(getChildFragmentManager());
                    break;
                //  @note 暂时关闭支持PC从机功能
//                case AttrAndFunCode.SYS_INFO_FUNCTION_PC_SLAVE:
////                    if (mCurrentControlFragment instanceof PCSlaveControlFragment) return;
//                    fragment = PCSlaveControlFragment.newInstanceForCache(getChildFragmentManager());
//                    break;
                default:
                    if (deviceInfo != null && deviceInfo.isBtEnable() && !deviceInfo.isSupportDoubleConnection()
                            && deviceInfo.getSdkType() != JLChipFlag.JL_COLOR_SCREEN_CHARGING_CASE) {
                        fun = AttrAndFunCode.SYS_INFO_FUNCTION_BT;
                        List<Music> musicList = mPresenter.getLocalMusicList(requireContext());
                        ...
                        fragment = BlankControlFragment.newInstanceForCache(getChildFragmentManager());
                        bundle = new Bundle();
                        bundle.putString(BlankControlFragment.KEY_CONTENT_TEXT, getString(R.string.no_data));
                    }
                    break;
            }
        }
        switchFunctionSelect(isConnect, fun);
        if (fragment != null) {
            if (fragment.isVisible() || isSameFragment(fragment)) return;
            mCurrentControlFragment = fragment;
            if (null != bundle && !fragment.isAdded()) {
    ...
    private boolean isSameFragment(Fragment fragment) {
        if (null == fragment || mCurrentControlFragment == null) return false;
        return Objects.equals(fragment.getClass().getCanonicalName(), mCurrentControlFragment.getClass().getCanonicalName());
    }
        ...
        if (!isSelect) {
            ...
            switch (fun) {
                ...
                case MultiMediaPresenterImpl.FRAGMENT_ID3_EMPTY:
                    ...
                    break;
//                case AttrAndFunCode.SYS_INFO_FUNCTION_SPDIF:
//                    mAdapter.setSelectedType(FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_SPDIF);
//                    break;
//                    @note 暂时关闭支持PC从机功能
//                case AttrAndFunCode.SYS_INFO_FUNCTION_PC_SLAVE:
//                    mAdapter.setSelectedType(FunctionItemData.FunctionItemType.FUNCTION_ITEM_TYPE_PC_SLAVE);
//                    break;
            }
        }
    }
