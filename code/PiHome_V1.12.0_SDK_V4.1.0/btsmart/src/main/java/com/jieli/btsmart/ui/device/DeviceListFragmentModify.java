 ...
 */
@RuntimePermissions
public class DeviceListFragmentModify extends BaseFragment implements NetworkStateHelper.Listener {
    ...
    public void requestLocationPermissionForScan() {
    ...
    public void showRelationForLocationPermission(PermissionRequest request) {
        disPermissionTipsDialog();
        showLocationPermissionTipsDialog(request);
    }
    ...
    public void onLocationDenied() {
        disPermissionTipsDialog();
        UIHelper.showAppSettingDialog(DeviceListFragmentModify.this, getString(R.string.permissions_tips_02) + getString(R.string.permission_location));
    }
    ...
    public void showBluetoothPermissionTips(PermissionRequest request) {
        disPermissionTipsDialog();
        showBluetoothPermissionTipsDialog(request);
    }
    ...
    public void bluetoothPermissionDenied() {
        disPermissionTipsDialog();
        UIHelper.showAppSettingDialog(DeviceListFragmentModify.this, getString(R.string.permissions_tips_02) + getString(R.string.permission_bluetooth));
    }

    private void tryToShowScanDeviceDialog() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) { //检查蓝牙权限
            if (!PermissionUtil.checkHasScanPermission(requireContext()) || !PermissionUtil.checkHasConnectPermission(requireContext())) {
                showPermissionTipsDialog(getString(R.string.connect_bt_permission));
                DeviceListFragmentModifyPermissionsDispatcher.requestBluetoothPermissionWithPermissionCheck(this);
                return;
            }
        }
        if(!PermissionUtil.isHasLocationPermission(requireContext())){ //检查位置权限
            showPermissionTipsDialog(getString(R.string.scan_bt_location_permission));
            DeviceListFragmentModifyPermissionsDispatcher.requestLocationPermissionForScanWithPermissionCheck(this);
            return;
        }
        showScanDeviceDialog();
    }
    ...
    private void showScanDeviceDialog() {
        ...
        if (!scanDeviceDialog.isShow()) {
            scanDeviceDialog.show(getChildFragmentManager(), ScanDeviceDialogModify.class.getCanonicalName());
        }
    }

    private void showLocationPermissionTipsDialog(PermissionRequest request) {
        Jl_Dialog dialog = Jl_Dialog.builder()
                .title(getString(R.string.dialog_tips))
                .content(getString(R.string.scan_bt_location_permission))
    ...
    private void showBluetoothPermissionTipsDialog(PermissionRequest request) {
        Jl_Dialog jl_dialog = Jl_Dialog.builder()
                .title(getString(R.string.dialog_tips))
                .content(getString(R.string.connect_bt_permission))
                .right(getString(R.string.setting))
                .rightColor(ContextCompat.getColor(requireContext(), R.color.blue_448eff))
                .rightClickListener((v, dialogFragment) -> {
                    dialogFragment.dismiss();
...
//                                mView.devNeedReboot();
    ...
    private void addReceiver() {
        if (mReceiver == null) {
            ...
            IntentFilter filter = new IntentFilter(SConstant.ACTION_DEVICE_CONNECTION_CHANGE);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                requireContext().registerReceiver(mReceiver, filter, Context.RECEIVER_NOT_EXPORTED);
            }else{
